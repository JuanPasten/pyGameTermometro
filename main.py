...
class NumberInput():
        ...
        def __init__(self, value = 0):
            ...
            '''
            #Este try y except es equivalente al de más arriba self.value(value)
            try:
        ...
        def on_event(self, event):
            if event.type == KEYDOWN:
                if event.unicode.isdigit() and len(self.__strValue) < 10 or (event.unicode == "." and self.__pointsCount == 0): # $ if event.unicode.isdigit() es equivalente a $ if event.unicode in "0123456789"
                    ...
                    #self.value = int(self.__strValue) #Esta línea es equivalente a la de abajo y consume menos ciclos, pero da error cuando la cadena queda vacía
                    self.value(self.__strValue)                    
...
class mainApp():
    ...
    def __init__(self):
        #Aquí el ancho y el largo son a gusto de la intergaz que te hayas creado
    ...
    def start(self):
        while True:
            ...
            text = self.entrada.render() #Obtenemos rectángulo blanco y foto del texto. Y se asigna a text
            ...
            self.__screen.blit(self.selector.custome(), (112, 152))
